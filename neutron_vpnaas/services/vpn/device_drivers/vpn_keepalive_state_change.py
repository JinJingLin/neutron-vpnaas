...
from neutron_vpnaas._i18n import _, _LE
from neutron_vpnaas.services.vpn.device_drivers import ha_ipsec

LOG = logging.getLogger(__name__)


class VPNKeepalivedUnixDomainConnection(agent_utils.UnixDomainHTTPConnection):
    def __init__(self, *args, **kwargs):
        # Old style super initialization is required!
        agent_utils.UnixDomainHTTPConnection.__init__(
            self, *args, **kwargs)
        self.socket_path = (
            ha_ipsec.VPNAgentKeepalivedStateChangeServer.
            get_vpn_keepalived_state_change_socket_path(cfg.CONF))


class MonitorDaemon(daemon.Daemon):
    def __init__(self, pidfile, vpnservice_id, user, group, namespace, conf_dir,
                 interface, cidr):
        ...
        self.cidr = cidr
        super(MonitorDaemon, self).__init__(pidfile, uuid=vpnservice_id,
    ...
    def parse_and_handle_event(self, iterable):
        try:
            ...
            if event.interface == self.interface and event.cidr == self.cidr:
                ...
                self.write_vpn_state_change(new_state)
                self.notify_vpn_agent(new_state)
        except Exception:
            LOG.exception(_LE(
                'Failed to process or handle event for line %s'), iterable)

    def write_vpn_state_change(self, state):
        with open(os.path.join(
                self.conf_dir, 'state'), 'w') as state_file:
            state_file.write(state)
        LOG.debug('Wrote vpnservice %s state %s', self.vpnservice_id, state)

    def notify_vpn_agent(self, state):
        resp, content = httplib2.Http().request(
            # Note that the message is sent via a Unix domain socket so that
            # the URL doesn't matter.
            'http://127.0.0.1/',
            headers={'X-Neutron-VPNaaS-Id': self.vpnservice_id,
                     'X-Neutron-VPNaaS-State': state},
            connection_type=VPNKeepalivedUnixDomainConnection)

        if resp.status != requests.codes.ok:
            raise Exception(_('Unexpected response: %s') % resp)

        LOG.debug('Notified agent vpnservice %s, state %s', self.vpnservice_id, state)


def register_opts(conf):
    conf.register_cli_opt(
        cfg.StrOpt('vpnservice_id', help=_('ID of the VPN service')))
    conf.register_cli_opt(
        cfg.StrOpt('namespace', help=_('Namespace of the VPN service')))
    conf.register_cli_opt(
        cfg.StrOpt('conf_dir', help=_('Path to the vpn directory')))
    conf.register_cli_opt(
    ...
    #TODO(bonnie) need to find out whether the metedata proxy is needed for vpn
    '''
    conf.register_opt(
